<template>
  <div id="testResults">
        <b-container fluid class="">         
            <b-row>
                <b-col style="margin-bottom: 15px;" v-for="(b, i) in belbin" :key="i+b.name"  sm="6">
                    <b-card id="belbin-details" :header="'Belbin Role ' + parseInt(i+1)" >
                        <h5>{{b.name}}</h5>
                        <b-card-text>
                ...
                <b-col style="margin-bottom: 15px;" v-for="(e, j) in enneagram" :key="j+j*j" sm="6">
                    <b-card id="enneagram-details" :header="'Enneagram type ' + parseInt(j+1)" >
                        <h5>{{e.name}}</h5>
                        <b-card-text>
                            <b-row class="list-item">
                                <b-col class="left" sm="4">
...
export default {
  ...
  computed: {
    ...mapGetters(["getLoginUser", "getAllUserDatasets", "getAllZodiacs", "getAllZodiacAscendants", "getAllEnneagrams", "getAllMbtis", "getAllBelbins", "getAllNumerologys"]),
  },
  methods: {
  },
  mounted() {
    let dataset = this.getAllUserDatasets.filter(ds => ds.userId === this.getLoginUser.id)[0];
