<template>
    <div id="testBelbin">       
            ...
            <b-row class="mb-3" v-for="q in questions" :key="q.questionId">
                <b-col sm="10" offset="1">      
                      <b-card title="" header-tag="header" footer-tag="footer">
                        <h6 slot="header" class="mb-0"> {{q.questionId}}) {{ q.questionText | capitalize}} </h6>
                        <b-card-text>
                        ...
                        <span class="float-right" slot="footer">Points remaining:  <b-badge :variant="q.remainingPoints === 0 ? 'success' : 'danger'" pill>{{ q.remainingPoints }}</b-badge> </span>
                    </b-card>
                </b-col>
            </b-row>
        ...
        <b-modal ref="my-modal" hide-footer header-bg-variant="warning" title="BELBIN Test completed successfully!">
            <div class="d-block text-center">
                <h3>Your BELBIN roles are: <span class="testColor"> {{belbinFirstRole}} </span> and <span class="testColor"> {{belbinSecondRole}} </span></h3>
            </div>
...
export default {
            ...
            if (this.formValid){
                this.questions.forEach((q, i) => {
                    q.answers.forEach((a, j) => {                        
                        ...
                        this.addUserBelbin(ub);
                        this.setUserBeblinSequance();
                    });
                });
    ...
    computed: {
        ...mapGetters(["getAllBelbinAnswers", "getAllBelbinQuestions", "getUserBelbinSequance", "getLoginUser", "getBelbinRoles", "getTestCompleted"]),
    },
    filters: {
