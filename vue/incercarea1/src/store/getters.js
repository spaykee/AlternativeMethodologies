export default {
    ...
    getBelbinRoles: (state) => {
        ...
        const userDataSet = state.userDataset.filter(ud => ud.userId === loginUserId)[0];

        return {
            first: typeof(userDataSet) !== "undefined" ? state.belbins.filter(b => b.id === userDataSet.belbin_first_role)[0].name : "",
            second: typeof(userDataSet) !== "undefined" ? state.belbins.filter(b => b.id === userDataSet.belbin_second_role)[0].name : ""
        }
    },
    ...
    getMbtiCompatibility: () => {
        let x = ['INFP', 'ENFP', 'INFJ', 'ENFJ', 'INTJ', 'ENTJ', 'INTP', 'ENTP', 'ISFP', 'ESFP', 'ISTP', 'ESTP', 'ISFJ', 'ESFJ', 'ISTJ', 'ESTJ'];
